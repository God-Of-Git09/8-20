...
const CHARACTERS = [
  ...
  { id: 'saber_jangyujun',  class: 'saber',     name: '장유준',       title: '봉인을 풀지 못한 자, 맨주먹의 사나이', color: '#ffb37a',
    mods: { dmg: 1.05, cd: 0.95, spd: 1.02, hp: 1.08 }, outfit: 'jangyujun',
    ultLine: '더는 참지 않는다... 해방!' },
...
function createPlayer(index, charId, x) {
  ...
  return {
    ...
    transformed: false, // 장유준 전용: 성검 해방(궁극기) 이후 영구적으로 true가 된다
...
function updateSkillsAndCombat() {
  for (const p of gameState.players) {
    ...
    if (!locked) {
      ...
      // 장유준은 맨주먹 상태(변신 전)에는 평타(J)가 아예 존재하지 않는다.
      if (p.input.attack && p.atkRecoveryTimer === 0 && !(isJY && !p.transformed)) {
        ...
        resetCombo(p);
        const rDur = isJY ? 22 : ATK_DUR.skill_r; // 냅다 박기/찌르기는 기본보다 조금 더 긴 모션
        p.action = 'skill_r'; p.actionTimer = rDur; p.cooldowns.r = Math.round(cd.r * p.cdMult);
      } else if (p.input.skill_f && p.cooldowns.f === 0 && !(isJY && p.transformed && p._jyLaserUsed)) {
        resetCombo(p);
        // 찍: 시전시간과 쿨타임이 매우 긴 강력한 한방 스킬. 변신 후에는 시전시간이 더욱 늘어나며 단 한 번만 사용 가능하다.
        const fDur = isJY ? (p.transformed ? 220 : 130) : ATK_DUR.skill_f;
        const fCdBase = isJY ? 60 * 13 : cd.f;
        p.action = 'skill_f'; p.actionTimer = fDur; p.cooldowns.f = Math.round(fCdBase * p.cdMult);
      } else if (p.input.skill_t && p.cooldowns.t === 0) {
        resetCombo(p);
        const tDur = isJY ? 78 : ATK_DUR.skill_t; // 잡고 박기/성검 회오리는 더 긴 모션이 필요하다
        p.action = 'skill_t'; p.actionTimer = tDur; p.cooldowns.t = Math.round(cd.t * p.cdMult);
      } else if (p.input.skill_x && p.ultGauge >= ULT_GAUGE_MAX && !(isJY && p.transformed)) {
        // 장유준은 한 번 해방(변신)하고 나면 궁극기를 다시 쓸 수 없다.
        resetCombo(p);
    ...
    } else if (p.charId === 'saber_jangyujun' && (p.action === 'skill_t' || p.action === 'skill_f')) {
      // 장유준의 U(잡고 박기/성검 회오리)와 I(찍)는 한 프레임 판정이 아니라
      // 여러 프레임에 걸친 다단계 모션이라 궁극기와 같은 방식(매 프레임 tick)으로 처리한다.
      resolveJangyujunTick(p, enemy);
...
function resolveCharSkill(p, enemy) {
  switch (p.charId) {
    ...
    case 'saber_jangyujun': switch (p.action) {
      case 'skill_e': // K - 싸기(원거리 포물선 레이저) / 성검 날리기(투척 후 순간이동)
        if (!p.transformed) {
          gameState.projectiles.push({ type: 'jy_beam', x: p.x + p.facing * 26, y: p.y - 34, vx: p.facing * 9.5, vy: -5.5, index: p.index, facing: p.facing, damage: D(p, 12), gravity: 0.32 });
        } else {
          gameState.projectiles.push({ type: 'jy_sword_throw', x: p.x + p.facing * 26, y: p.y - 40, vx: p.facing * 15, vy: 0, index: p.index, facing: p.facing, damage: D(p, 14), pierce: true });
          p.invincible = Math.max(p.invincible, 14);
          p.x = clampX(p.x + p.facing * 180);
          gameState.effects.push({ type: 'transform_out', x: p.x, y: p.y - 40, life: 16, maxLife: 16, color: p.color });
          if (meleeHit(p, enemy, 80)) applyDamage(p, enemy, D(p, 8));
        }
        break;
      case 'skill_r': // L - 냅다 박기(돌진 박치기, "영차") / 찌르기(성검 돌진 찌르기)
        if (!p.transformed) {
          p.x = clampX(p.x + p.facing * 100);
          gameState.effects.push({ type: 'dash', x: p.x, y: p.y - 30, life: 14, maxLife: 14, color: p.color });
          gameState.effects.push({ type: 'jy_shout', x: p.x, y: p.y - 92, life: 26, maxLife: 26, color: p.color, text: '영차!' });
          if (meleeHit(p, enemy, 88)) { applyDamage(p, enemy, D(p, 12)); if (enemy.hp > 0) applyStun(enemy, 18); }
        } else {
          p.x = clampX(p.x + p.facing * 95);
          gameState.effects.push({ type: 'dash', x: p.x, y: p.y - 30, life: 14, maxLife: 14, color: p.color });
          if (meleeHit(p, enemy, 92)) applyDamage(p, enemy, D(p, 15));
        }
        break;
    } break;
...
// ─── 장유준 전용: U(잡고 박기/성검 회오리), I(찍) 다단계 모션 처리 ───────────
// 이 두 스킬은 한 프레임짜리 판정이 아니라 여러 단계(붙잡기→흔들기→마무리,
// 혹은 긴 예비동작→발사)로 이루어져 있어 궁극기와 동일한 방식(매 프레임 tick)으로 처리한다.
function resolveJangyujunTick(p, enemy) {
  const f = p.facing;
  ...
  if (p.action === 'skill_t') {
    ...
    if (!p.transformed) {
      // 잡고 박기: 손을 뻗어 붙잡으면 적을 눈앞으로 순간이동시키고 스턴, 이후 허리를 앞뒤로 흔들며 두들긴다("영차")
      if (frame === 10 && !p._jyGrabbed && meleeHit(p, enemy, 95)) {
        p._jyGrabbed = true;
        enemy.x = clampX(p.x + f * 34);
        applyStun(enemy, 70);
        gameState.effects.push({ type: 'counter', x: enemy.x, y: enemy.y - 40, life: 16, maxLife: 16, color: p.color });
      }
      if (p._jyGrabbed && [28, 42, 56].includes(frame) && enemy) {
        applyDamage(p, enemy, D(p, 6));
        gameState.effects.push({ type: 'jy_shout', x: p.x, y: p.y - 92, life: 20, maxLife: 20, color: p.color, text: '영차!' });
      }
      if (frame === 70) {
        if (p._jyGrabbed && enemy) {
          applyDamage(p, enemy, D(p, 10));
          gameState.effects.push({ type: 'hit', x: enemy.x, y: enemy.y - 40, life: 12, maxLife: 12 });
        }
        p._jyGrabbed = false;
      }
    } else {
      // 성검 회오리: 전방위로 성검을 휘두른다
      if (frame === 14 && meleeHit(p, enemy, 105)) {
        applyDamage(p, enemy, D(p, 16));
        if (enemy.hp > 0) applyStun(enemy, 26);
        gameState.effects.push({ type: 'whirl', x: p.x, y: p.y - 40, life: 20, maxLife: 20, color: p.color });
      }
    }
  } else if (p.action === 'skill_f') {
    const total = p.transformed ? 220 : 130;
    const frame = total - p.actionTimer;
    // 찍: 손을 허리 앞에서 두 번(변신 후엔 네 번) 왔다갔다 한 뒤 강력한 레이저를 발사한다
    if (frame === 0) gameState.effects.push({ type: 'ult_charge', x: p.x, y: p.y - 40, life: 24, maxLife: 24, color: p.color });
    const windupFrames = p.transformed ? [30, 70, 110, 150] : [24, 48];
    if (windupFrames.includes(frame)) gameState.effects.push({ type: 'jy_windup', x: p.x + f * 20, y: p.y - 40, life: 14, maxLife: 14, color: p.color });
    const releaseFrame = p.transformed ? 190 : 62;
    if (frame === releaseFrame) {
      if (!p.transformed) {
        gameState.projectiles.push({ type: 'jy_laser', x: p.x + f * 30, y: p.y - 40, vx: f * 16, vy: 0, index: p.index, facing: f, damage: D(p, 30), pierce: true, finisher: true });
        gameState.effects.push({ type: 'excalibur_strike', x: p.x + f * 40, y: p.y - 40, life: 26, maxLife: 26, color: p.color });
      } else {
        // 변신 후: 화면 절반을 가릴 정도로 강력한 레이저 (변신 후 단 한 번만 사용 가능)
        for (let i = -1; i <= 1; i++) {
          gameState.projectiles.push({ type: 'jy_laser_big', x: p.x + f * 30, y: p.y - 40 + i * 26, vx: f * 18, vy: 0, index: p.index, facing: f, damage: D(p, 22), pierce: true, finisher: true });
        }
        gameState.effects.push({ type: 'laser', x: p.x + f * 30, y: p.y - 40, life: 30, maxLife: 30, color: p.color, extra: f });
        p._jyLaserUsed = true;
      }
...
function resolveUltimateTick(p, enemy) {
  ...
  switch (p.charId) {
    ...
    // 장유준 — 성검 해방 (위로 높게 뛰어오른 뒤, 봉인되어 있던 긴 성검을 뽑아 들고 착지한다.
    // 착지와 동시에 영구적으로 변신 상태가 되어 이후 스킬셋이 전부 바뀐다)
    case 'saber_jangyujun': {
      if (frame === 0) {
        p.invincible = Math.max(p.invincible, 55);
        p._jyBaseY = p.isGrounded ? p.y : (gameState.map ? gameState.map.floorY : FLOOR_Y);
        gameState.effects.push({ type: 'ult_charge', x: p.x, y: p.y - 10, life: 20, maxLife: 20, color: p.color });
      }
      // 위로 높게 점프 (0~50프레임)
      if (frame > 0 && frame <= 50) {
        const k = frame / 50;
        p.isGrounded = false;
        p.y = p._jyBaseY - Math.sin(Math.PI * k) * 150;
      }
      if (frame === 50) {
        gameState.effects.push({ type: 'jy_reveal', x: p.x, y: p.y - 40, life: 46, maxLife: 46, color: p.color });
        gameState.hitStop = Math.max(gameState.hitStop, 6);
      }
      // 공중에서 성검을 뽑아든 채 정지 (50~90)
      if (frame > 50 && frame < 90) {
        p.isGrounded = false;
        p.y = p._jyBaseY - 150;
      }
      // 착지 (90~total)
      if (frame >= 90 && frame < total) {
        const k = (frame - 90) / Math.max(1, total - 90);
        p.isGrounded = false;
        p.y = (p._jyBaseY - 150) + 150 * Math.min(1, k * 1.15);
      }
      if (frame === total - 2) {
        p.y = p._jyBaseY; p.isGrounded = true;
        p.transformed = true;
        gameState.effects.push({ type: 'ultimate', x: p.x, y: p.y - 40, life: 26, maxLife: 26, color: p.color });
      }
      break;
    }
